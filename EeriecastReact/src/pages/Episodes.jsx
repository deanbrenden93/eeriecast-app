...
/**
 * Parse the optional `t=<seconds>` deep-link param.
 *
 * Accepts a bare integer ("123") or the common "1h2m3s" / "02:13"
 * shorthands that show up in shared podcast links and YouTube-style
 * embeds, so a link built by any sharing tool Just Works.
 */
...
export default function Episodes() {
  ...
  // Optional deep-link: ?ep=<episodeId>&t=<seconds> — auto-opens the
  // player with that specific episode queued + playing. `t` is honored
  // as a resume position so a shared "highlight at 12:30" link starts
  // at the exact moment the sharer meant.
  const deepLinkEpId = query.get('ep') || query.get('episode');
  const deepLinkStart = parseTimestampSeconds(query.get('t') || query.get('start'));
  ...
  const doPlay = async (ep, opts = {}) => {
    if (!ep) return;
    const startSeconds = Number(opts.startSeconds) > 0 ? Number(opts.startSeconds) : 0;
    ...
    try {
      ...
      // Honour ?t=<seconds> deep-link: start playback at the caller's
      // offset on the target episode only. All other queue items still
      // start from the beginning when autoplay advances to them.
      if (startSeconds > 0 && startIdx >= 0 && queueItems[startIdx]) {
        queueItems[startIdx].resume = { progress: startSeconds };
      }
      ...
      if (queueItems.length > 0 && startIdx >= 0) {
        ...
        const played = await loadAndPlay({
          podcast: show,
          episode: playEp,
          resume: startSeconds > 0 ? { progress: startSeconds } : undefined,
        });
        if (played === false) {
      ...
      try { await UserLibrary.addToHistory(playEp.id, startSeconds); } catch (e) { if (typeof console !== 'undefined') console.debug('history add failed', e); }
    } catch (e) {
  ...
  // ── Deep-link auto-play ──────────────────────────────────────────
  // If the URL carries ?ep=<id>[&t=<seconds>] (as generated by share.js
  // and, in practice, by any external site sharing an episode), find
  // that episode in the loaded list and start it playing automatically.
  //
  // Behaviour rules:
  //   • Only fires once per (show, episode) pair — navigating back or
  //     toggling tabs won't re-trigger it.
  //   • Holds off while the mature-gate modal is up so we don't
  //     autoplay audio the user hasn't confirmed they want to hear.
  //   • Locked / premium-only episodes route through the normal gates
  //     in doPlay, which will bounce free users to /Premium — but only
  //     if the episode isn't a free sample (that's what doPlay already
  //     enforces via lockedEpisodeIds / sampleEpisodeIds).
  //   • Works for new episodes automatically because we match by id
  //     against the freshly-fetched `episodes` list.
  const autoPlayedRef = useRef(null);
  useEffect(() => {
    if (!deepLinkEpId) return;
    if (showMatureGate) return; // wait for the gate to be cleared
    if (!show || episodes.length === 0) return;
    const key = `${show.id}:${deepLinkEpId}`;
    if (autoPlayedRef.current === key) return;

    const target = episodes.find(
      (e) => String(e?.id) === String(deepLinkEpId) || e?.slug === deepLinkEpId,
    );
    if (!target) return;
